...
{       
    if (enablelighting) {       
		...
		vec4 transformed = modelview * myvertex;
        vec3 mypos = transformed.xyz / transformed.w ;
		...
		for (int i = 0; i < numLights; i++) {
		    // if directional
			if (lightposn[i].w == 0) {
				vec3 lightDirn = normalize(lightposn[i].xyz);
				vec3 halfAngle = normalize(lightDirn + eyedirn); 
				finalcolor += ComputeLight(lightDirn, lightcolor[i], normal, halfAngle, diffuse, specular, shininess);
			}
			// if point
			else {
				vec3 position = lightposn[i].xyz / lightposn[i].w; 
				vec3 lightDirn = normalize(position - mypos);
				vec3 halfAngle = normalize(lightDirn + eyedirn);
				finalcolor += ComputeLight(lightDirn, lightcolor[i], normal, halfAngle, diffuse, specular, shininess);
			}
		}
